...
output:
  html_document:
    toc: TRUE
    toc_depth: 2  # Numeric
    toc_float: TRUE # Do you want TOC on the side?
    code_download: TRUE  # Embed Rmd source code into html? 
    code_folding: "show"
    theme: "spacelab"
    highlight: "tango"  # theme for syntax highlight
---
...
**<span style="color:red">IMPORTANT NOTE</span>: ** As this is a transcription by a student who is still studying IRT for his own notes, there will be errors in various parts. Please let me know if there are any mistakes.
...
\begin{align}
\alpha_j &\sim N(a_0, A_0)\tag{3}\\
\beta_j &\sim N(b_0, B_0)\tag{4}\\
\theta_i &\sim N(0, 1)\tag{5}.
\end{align}

Then, we can write the conditional posterior distribution for $\alpha, \beta, \theta$ from equation (1) ~ (5), given the rest parameters:
...
Normally, we would use a Gibbs sampler to sample parameters from the posterior distribution, but since the conditional posteriors in (6) ~ (8) above are not in the form of the standard normal or gamma distributions, we cannot simply sample parameters using a Gibbs sampler. Therefore, we adopt the Metropolis-Hastings Algorithm, which allows us to perform MCMC even in such a case.
...
Very unfortunately, it is many times faster to do IRT with MCMCpack. Also, it seems that MCMCpack uses probit execution instead of logit. I don't know if that affects the execution time, but it does seem very fast. Let's check the result.
